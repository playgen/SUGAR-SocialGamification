...
namespace PlayGen.SUGAR.Server.WebAPI.Controllers
{
	...
	public class RoleClaimController : Controller
	{
		...
		/// </summary>
		/// <returns>A list of <see cref="ClaimResponse"/> that holds Claim details.</returns>
		[HttpGet("role/{id:int}")]
		...
		public async Task<IActionResult> GetRoleClaims([FromRoute]int id)
		{
			...
			{
				var claims = _roleClaimCoreController.GetClaimsByRole(id);
				var claimContract = claims.ToContractList();
