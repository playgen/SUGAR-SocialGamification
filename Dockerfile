...
RUN apt-get update && apt-get -y install unzip curl libicu-dev libunwind8 gettext libssl-dev libcurl3-gnutls zlib1g  sqlite3 libsqlite3-dev npm && rm -rf /var/lib/apt/lists/*
...
COPY src app/
WORKDIR app/PlayGen.SUGAR.WebAPI/
RUN ["dotnet", "restore"]

EXPOSE 5004
ENTRYPOINT ["dotnet", "-p", "project.json", "web"]