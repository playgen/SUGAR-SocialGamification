...
namespace PlayGen.SUGAR.Core.EvaluationEvents
{
	...
	public class CriteriaEvaluator
	{
		...
		public CriteriaEvaluator(SUGARContextFactory contextFactory, GroupMemberController groupMemberCoreController,
		...
		public float IsCriteriaSatisified(int? gameId, int? actorId, List<EvaluationCriteria> completionCriterias,
			ActorType actorType)
		{
		...
		{
			var evaluationDataController =
				new EvaluationDataController(ContextFactory, completionCriteria.EvaluationDataCategory);

			switch (completionCriteria.CriteriaQueryType)
			{
				case CriteriaQueryType.Any:
					var any = evaluationDataController.AllLongs(gameId, actorId, completionCriteria.EvaluationDataKey)
						.ToList();

					return !any.Any()
						? 0
						: any.Max(value => CompareValues(value,
							long.Parse(completionCriteria.Value),
							completionCriteria.ComparisonType,
							completionCriteria.EvaluationDataType));
				case CriteriaQueryType.Sum:
					var sum = evaluationDataController.SumLongs(gameId, actorId, completionCriteria.EvaluationDataKey);

					return CompareValues(sum,
						long.Parse(completionCriteria.Value),
						completionCriteria.ComparisonType,
						completionCriteria.EvaluationDataType);
		...
		protected float EvaluateFloat(int? gameId, int? actorId, EvaluationCriteria completionCriteria)
		{
			...
			{
				case CriteriaQueryType.Any:
					var any = evaluationDataController.AllFloats(gameId, actorId, completionCriteria.EvaluationDataKey)
						.ToList();

					return !any.Any()
						? 0
						: any.Max(value => CompareValues(value,
							float.Parse(completionCriteria.Value),
							completionCriteria.ComparisonType,
							completionCriteria.EvaluationDataType));
				case CriteriaQueryType.Sum:
		...
		protected float EvaluateString(int? gameId, int? actorId, EvaluationCriteria completionCriteria)
		{
			...
			{
				case CriteriaQueryType.Any:
					var any = evaluationDataController.AllStrings(gameId, actorId, completionCriteria.EvaluationDataKey)
						.ToList();
				...
				case CriteriaQueryType.Latest:
					string latest;
					if (!evaluationDataController.TryGetLatestString(gameId,
						actorId,
						completionCriteria.EvaluationDataKey,
						out latest))
						return 0;
		...
		protected float EvaluateBool(int? gameId, int? actorId, EvaluationCriteria completionCriteria)
		{
			...
			{
				case CriteriaQueryType.Any:
					var any = evaluationDataController.AllBools(gameId, actorId, completionCriteria.EvaluationDataKey)
						.ToList();

					return !any.Any()
						? 0
						: any.Max(value => CompareValues(value,
							bool.Parse(completionCriteria.Value),
							completionCriteria.ComparisonType,
							completionCriteria.EvaluationDataType));
				case CriteriaQueryType.Latest:
					bool latest;
		...
		protected static float CompareValues<T>(T value, T expected, ComparisonType comparisonType,
