...
namespace PlayGen.SUGAR.Core.EvaluationEvents
{
	public class EvaluationTracker : IDisposable
	{
		...
		private readonly EvaluationEvaluationDataMapper _EvaluationDataToEvaluationMapper =
			new EvaluationEvaluationDataMapper();
		...
		public ConcurrentDictionary<int, ConcurrentDictionary<Evaluation, float>> GetPendingNotifications(int? gameId,
			int actorId)
		{
		...
		{
			var evaluations = _evaluationController.GetByGame(gameId)
				.ToList();
