...
namespace PlayGen.SUGAR.Core.Controllers
{
	public class LeaderboardController : CriteriaEvaluator
	{
		...
		public List<LeaderboardStandingsResponse> GetStandingsList(Leaderboard leaderboard, LeaderboardStandingsRequest request)
		{
			if (leaderboard == null)
			...
			var standings = GetStandings(leaderboard, request);
		...
		protected List<LeaderboardStandingsResponse> GetStandings(Leaderboard leaderboard, LeaderboardStandingsRequest request)
		{
			...
			switch (leaderboard.LeaderboardType)
			{
				case LeaderboardType.Highest:
					typeResults = EvaluateHighest(evaluationDataController,
						actors,
						leaderboard.GameId,
				...
				case LeaderboardType.Lowest:
					typeResults = EvaluateLowest(evaluationDataController,
						actors,
						leaderboard.GameId,
						leaderboard.EvaluationDataKey,
						leaderboard.LeaderboardType,
				...
				case LeaderboardType.Cumulative:
					typeResults = EvaluateCumulative(evaluationDataController,
						actors,
						leaderboard.GameId,
						leaderboard.EvaluationDataKey,
						leaderboard.LeaderboardType,
				...
				case LeaderboardType.Count:
					typeResults = EvaluateCount(evaluationDataController,
						actors,
						leaderboard.GameId,
						leaderboard.EvaluationDataKey,
						leaderboard.LeaderboardType,
				...
				case LeaderboardType.Earliest:
					typeResults = EvaluateEarliest(evaluationDataController,
						actors,
						leaderboard.GameId,
						leaderboard.EvaluationDataKey,
						leaderboard.LeaderboardType,
				...
				case LeaderboardType.Latest:
					typeResults = EvaluateLatest(evaluationDataController,
						actors,
						leaderboard.GameId,
						leaderboard.EvaluationDataKey,
						leaderboard.LeaderboardType,
			...
			var results = FilterResults(typeResults,
				request.PageLimit,
				request.PageOffset,
				request.LeaderboardFilterType,
				request.ActorId);
		...
		{
			var actors = Enumerable.Empty<ActorResponse>()
				.ToList();
		...
		protected List<LeaderboardStandingsResponse> EvaluateHighest(EvaluationDataController evaluationDataController,
			List<ActorResponse> actors, int? gameId, string key, LeaderboardType type, EvaluationDataType evaluationDataType,
			LeaderboardStandingsRequest request)
		{
					...
					results = actors.Select(r => new LeaderboardStandingsResponse
						{
							ActorId = r.Id,
							ActorName = r.Name,
							Value = evaluationDataController.GetHighestLongs(gameId, r.Id, key, request.DateStart, request.DateEnd)
								.ToString()
						})
						.ToList();
					break;
					...
					results = actors.Select(r => new LeaderboardStandingsResponse
						{
							ActorId = r.Id,
							ActorName = r.Name,
							Value = evaluationDataController.GetHighestFloats(gameId, r.Id, key, request.DateStart, request.DateEnd)
								.ToString()
						})
						.ToList();
					break;
			...
			Logger.Debug(
				$"{results?.Count} Actors for GameId: {gameId}, Key: {key}, Leaderboard Type: {type}, Save Data Type: {evaluationDataType}");

			return results;
		}

		protected List<LeaderboardStandingsResponse> EvaluateLowest(EvaluationDataController evaluationDataController,
			List<ActorResponse> actors, int? gameId, string key, LeaderboardType type, EvaluationDataType evaluationDataType,
			LeaderboardStandingsRequest request)
		{
			List<LeaderboardStandingsResponse> results;
					...
					results = actors.Select(r => new LeaderboardStandingsResponse
						{
							ActorId = r.Id,
							ActorName = r.Name,
							Value = evaluationDataController.GetLowestLongs(gameId, r.Id, key, request.DateStart, request.DateEnd)
								.ToString()
						})
						.ToList();
					break;
					...
					results = actors.Select(r => new LeaderboardStandingsResponse
						{
							ActorId = r.Id,
							ActorName = r.Name,
							Value = evaluationDataController.GetLowestFloats(gameId, r.Id, key, request.DateStart, request.DateEnd)
								.ToString()
						})
						.ToList();
					break;
		...
		protected List<LeaderboardStandingsResponse> EvaluateCumulative(EvaluationDataController evaluationDataController,
			List<ActorResponse> actors, int? gameId, string key, LeaderboardType type, EvaluationDataType evaluationDataType,
			LeaderboardStandingsRequest request)
		{
			List<LeaderboardStandingsResponse> results;
					...
					results = actors.Select(r => new LeaderboardStandingsResponse
						{
							ActorId = r.Id,
							ActorName = r.Name,
							Value = evaluationDataController.SumLongs(gameId, r.Id, key, request.DateStart, request.DateEnd)
								.ToString()
						})
						.ToList();
					break;
					...
					results = actors.Select(r => new LeaderboardStandingsResponse
						{
							ActorId = r.Id,
							ActorName = r.Name,
							Value = evaluationDataController.SumFloats(gameId, r.Id, key, request.DateStart, request.DateEnd)
								.ToString()
						})
						.ToList();
					break;
		...
		protected List<LeaderboardStandingsResponse> EvaluateCount(EvaluationDataController evaluationDataController,
			List<ActorResponse> actors, int? gameId, string key, LeaderboardType type, EvaluationDataType evaluationDataType,
			LeaderboardStandingsRequest request)
		{
			List<LeaderboardStandingsResponse> results;
					...
					results = actors.Select(r => new LeaderboardStandingsResponse
						{
							ActorId = r.Id,
							ActorName = r.Name,
							Value = evaluationDataController.CountKeys(gameId,
									r.Id,
									key,
									evaluationDataType,
									request.DateStart,
		...
		protected List<LeaderboardStandingsResponse> EvaluateEarliest(EvaluationDataController evaluationDataController,
			List<ActorResponse> actors, int? gameId, string key, LeaderboardType type, EvaluationDataType evaluationDataType,
			LeaderboardStandingsRequest request)
		{
			List<LeaderboardStandingsResponse> results;
					...
					results = actors.Select(r => new LeaderboardStandingsResponse
						{
							ActorId = r.Id,
							ActorName = r.Name,
							Value = evaluationDataController.TryGetEarliestKey(gameId,
									r.Id,
									key,
									evaluationDataType,
									request.DateStart,
		...
		protected List<LeaderboardStandingsResponse> EvaluateLatest(EvaluationDataController evaluationDataController,
			List<ActorResponse> actors, int? gameId, string key, LeaderboardType type, EvaluationDataType evaluationDataType,
			LeaderboardStandingsRequest request)
		{
			List<LeaderboardStandingsResponse> results;
					...
					results = actors.Select(r => new LeaderboardStandingsResponse
						{
							ActorId = r.Id,
							ActorName = r.Name,
							Value = evaluationDataController.TryGetLatestKey(gameId,
									r.Id,
									key,
									evaluationDataType,
									request.DateStart,
		...
		protected List<LeaderboardStandingsResponse> FilterResults(List<LeaderboardStandingsResponse> typeResults, int limit,
			int offset, LeaderboardFilterType filter, int? actorId)
		{
			int position;
			switch (filter)
			{
				case LeaderboardFilterType.Top:
					typeResults = typeResults.Skip(offset * limit)
						.Take(limit)
						.ToList();
					...
					return typeResults.Select(s => new LeaderboardStandingsResponse
						...
						.ToList();
				case LeaderboardFilterType.Near:
					var typeResultList = typeResults ?? typeResults.ToList();
					var actorCheck = actorId != null && typeResultList.Any(r => r.ActorId == actorId.Value);
					if (actorCheck)
					{
						var actorPosition = typeResultList.TakeWhile(r => r.ActorId != actorId.Value)
							.Count();
						offset += actorPosition / limit;
					}
					typeResults = typeResultList.Skip(offset * limit)
						.Take(limit)
						.ToList();
					position = offset * limit;
					return typeResults.Select(s => new LeaderboardStandingsResponse
				...
				case LeaderboardFilterType.Friends:
					...
					if (actorId != null)
					{
						var friends = UserFriendCoreController.GetFriends(actorId.Value)
							.Select(r => r.Id)
							.ToList();
						friends.Add(actorId.Value);
						var friendsOnly = overall.Where(r => friends.Contains(r.ActorId));
						friendsOnly = friendsOnly.Skip(offset * limit)
							.Take(limit);
				...
				case LeaderboardFilterType.GroupMembers:
					...
					if (actorId != null)
					{
						var members = GroupMemberCoreController.GetMembers(actorId.Value)
							.Select(r => r.Id);
						var membersOnly = all.Where(r => members.Contains(r.ActorId));
						membersOnly = membersOnly.Skip(offset * limit)
							.Take(limit);
