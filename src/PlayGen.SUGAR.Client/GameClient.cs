...
namespace PlayGen.SUGAR.Client
{
	...
	public class GameClient : ClientBase
	{
		...
		public GameClient(string baseAddress, IHttpHandler httpHandler, AsyncRequestController asyncRequestController,
		...
		/// <summary>
		///     Get a list of Games that match
		///     <param name="name" />
		///     provided.
		/// </summary>
		...
		public GameResponse Get(int id)
		{
			var query = GetUriBuilder(ControllerPrefix + "/findbyid/{0}", id)
				.ToString();
			return Get<GameResponse>(query, new[] {HttpStatusCode.OK, HttpStatusCode.NoContent});
		}
