...
namespace PlayGen.SUGAR.Client
{
	...
	public class AchievementClient : ClientBase
	{
		...
		public AchievementClient(string baseAddress, IHttpHandler httpHandler, AsyncRequestController asyncRequestController,
		...
		/// <summary>
		///     Find a Global Achievement that matches
		///     <param name="token" />
		///     .
		/// </summary>
		...
		/// <summary>
		///     Find a Achievement that matches
		///     <param name="token" />
		///     and
		///     <param name="gameId" />
		///     .
		...
		/// <summary>
		///     Find a list of Achievements that match
		///     <param name="gameId" />
		///     .
		/// </summary>
		...
		/// <summary>
		///     Find the current progress for all global achievements for
		///     <param name="actorId" />
		///     .
		/// </summary>
		...
		/// <summary>
		///     Find the current progress for all achievements for a
		///     <param name="gameId" />
		///     for
		///     <param name="actorId" />
		///     .
		...
		{
			var query = GetUriBuilder(ControllerPrefix + "/{0}/global/evaluate/{1}", token, actorId)
				.ToString();
		...
		{
			var query = GetUriBuilder(ControllerPrefix + "/{0}/{1}/evaluate/{2}", token, gameId, actorId)
				.ToString();
			return Get<EvaluationProgressResponse>(query);
		}
		...
		///     Requires <see cref="EvaluationCreateRequest.Name" /> to be unique to that
		///     <see cref="EvaluationCreateRequest.GameId" />.
		/// </summary>
		/// <param name="newAchievement">
		///     <see cref="EvaluationCreateRequest" /> object that holds the details of the new
		///     Achievement.
		/// </param>
		...
		{
			var query = GetUriBuilder(ControllerPrefix + "/update")
				.ToString();
		...
		{
			var query = GetUriBuilder(ControllerPrefix + "/{0}/global", token)
				.ToString();
			Delete(query);
		}

		/// <summary>
		///     Delete Achievement with the
		///     <param name="token" />
		///     and
		///     <param name="gameId" />
		///     provided.
		...
		{
			var query = GetUriBuilder(ControllerPrefix + "/{0}/{1}", token, gameId)
				.ToString();
			Delete(query);
		}
