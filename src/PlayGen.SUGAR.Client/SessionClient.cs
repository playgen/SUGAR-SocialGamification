...
namespace PlayGen.SUGAR.Client
{
	...
	public class SessionClient : ClientBase
	{
		...
		public SessionClient(string baseAddress, IHttpHandler httpHandler, AsyncRequestController asyncRequestController,
		...
		{
			var query = GetUriBuilder(ControllerPrefix + "/login")
				.ToString();
		...
		{
			var query = GetUriBuilder(ControllerPrefix + "/{0}/login", gameId)
				.ToString();
			return Post<AccountRequest, AccountResponse>(query, account);
		}

		public void LoginAsync(int gameId, AccountRequest account, Action<AccountResponse> onSuccess,
		...
		{
			var query = GetUriBuilder(ControllerPrefix + "/createandlogin")
				.ToString();
		...
		{
			var query = GetUriBuilder(ControllerPrefix + "/{0}/createandlogin", gameId)
				.ToString();
			return Post<AccountRequest, AccountResponse>(query, accountRequest);
		}
		...
		{
			var query = GetUriBuilder(ControllerPrefix + "/heartbeat")
				.ToString();
			Get(query);
		}
		...
		public void Logout()
		{
			var query = GetUriBuilder(ControllerPrefix + "/logout")
				.ToString();
			Get(query);
