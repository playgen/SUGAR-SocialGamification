...
namespace PlayGen.SUGAR.Client
{
	...
	public class LeaderboardClient : ClientBase
	{
		...
		public LeaderboardClient(string baseAddress, IHttpHandler httpHandler, AsyncRequestController asyncRequestController,
		...
		/// <summary>
		///     Find a list of leaderboards that match
		///     <param name="gameId" />
		///     .
		/// </summary>
		...
		public LeaderboardResponse GetGlobal(string token)
		{
			var query = GetUriBuilder(ControllerPrefix + "/{0}/global", token)
				.ToString();
			return Get<LeaderboardResponse>(query, new[] {HttpStatusCode.OK, HttpStatusCode.NoContent});
		}
		...
		///     Requires <see cref="LeaderboardRequest.Name" /> and <see cref="LeaderboardRequest.Token" /> to be unique to that
		///     <see cref="LeaderboardRequest.GameId" />.
		/// </summary>
		...
		/// </summary>
		/// <param name="leaderboardDetails">
		///     <see cref="LeaderboardStandingsRequest" /> object that holds the details that are
