...
namespace PlayGen.SUGAR.Client
{
	...
	public class MatchClient : ClientBase
	{
		...
		public MatchClient(string baseAddress, IHttpHandler httpHandler, AsyncRequestController asyncRequestController,
		...
		{
			var query = GetUriBuilder(ControllerPrefix + "/createandstart")
				.ToString();
			return Get<MatchResponse>(query);
		}
		...
		{
			var query = GetUriBuilder(ControllerPrefix + "/{0}/start")
				.ToString();
			return Get<MatchResponse>(query);
		}
		...
		{
			var query = GetUriBuilder(ControllerPrefix + "/{0}/end", matchId)
				.ToString();
			return Get<MatchResponse>(query);
		}
