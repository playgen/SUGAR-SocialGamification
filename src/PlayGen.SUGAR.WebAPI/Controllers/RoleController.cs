...
namespace PlayGen.SUGAR.WebAPI.Controllers
{
	...
	public class RoleController : Controller
	{
		...
		public IActionResult GetByScope([FromRoute] string name)
		{
			...
			if (Enum.TryParse(name, true, out claimScope))
			{
				if (_authorizationService.AuthorizeAsync(User,
						claimScope,
						(AuthorizationRequirement) HttpContext.Items[claimScope + "Requirements"])
					.Result)
				{
					var roles = _roleCoreController.GetByScope(claimScope);
					return new ObjectResult(roles.ToCollectionContract());
				}
				return Forbid();
			}
			return Forbid();
		...
		public IActionResult Create([FromBody] RoleRequest newRole)
		{
			if (_authorizationService.AuthorizeAsync(User,
					newRole.ClaimScope,
					(AuthorizationRequirement) HttpContext.Items[newRole.ClaimScope + "Requirements"])
				.Result)
			{
