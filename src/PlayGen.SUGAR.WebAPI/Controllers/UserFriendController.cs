...
namespace PlayGen.SUGAR.WebAPI.Controllers
{
	...
	public class UserFriendController : Controller
	{
		...
		/// <summary>
		/// </summary>
		/// <param name="userFriendCoreController"></param>
		...
		{
			if (_authorizationService.AuthorizeAsync(User, userId, (AuthorizationRequirement) HttpContext.Items["Requirements"])
				.Result)
			{
				var friendRequests = _userFriendCoreController.GetFriendRequests(userId);
				var actorContract = ActorExtensions.ToCollectionContract(friendRequests);
		...
		public IActionResult GetSentRequests([FromRoute] int userId)
		{
			if (_authorizationService.AuthorizeAsync(User, userId, (AuthorizationRequirement) HttpContext.Items["Requirements"])
				.Result)
			{
				var sentRequests = _userFriendCoreController.GetSentRequests(userId);
				var actorContract = ActorExtensions.ToCollectionContract(sentRequests);
				return new ObjectResult(actorContract);
			}
			return Forbid();
		}
		...
		{
			var friends = _userFriendCoreController.GetFriends(userId);
			var actorContract = ActorExtensions.ToCollectionContract(friends);
			return new ObjectResult(actorContract);
		}
		...
		{
			if (_authorizationService.AuthorizeAsync(User, relationship.RequestorId, (AuthorizationRequirement) HttpContext.Items["Requirements"])
					.Result
				|| _authorizationService.AuthorizeAsync(User, relationship.AcceptorId, (AuthorizationRequirement) HttpContext.Items["Requirements"])
					.Result)
			{
		...
		///     <param name="relationship.RequestorId" />
		///     and
		///     <param name="relationship.AcceptorId" />
		///     .
