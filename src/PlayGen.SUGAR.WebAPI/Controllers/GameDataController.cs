...
namespace PlayGen.SUGAR.WebAPI.Controllers
{
	...
	public class GameDataController : Controller
	{
		...
		/// <summary>
		///     Find a list of all GameData that match the
		///     <param name="actorId" />
		///     ,
		///     <param name="gameId" />
		///     and
		...
		public IActionResult Get(int? actorId, int? gameId, string[] key)
		{
			if (_authorizationService.AuthorizeAsync(User, gameId, (AuthorizationRequirement) HttpContext.Items["Requirements"]).Result)
			{
				...
				var dataContract = data.ToCollectionContract();
		...
		/// <summary>
		///     Find a list of all Actors that have data saved for the game
		///     <param name="id" />
		///     provided.
		...
		/// <summary>
		///     Find a list of all GameData keys for the
		///     <param name="id" />
		///     provided.
		...
		{
			if (_authorizationService.AuthorizeAsync(User, id, (AuthorizationRequirement) HttpContext.Items["GroupRequirements"])
					.Result ||
				_authorizationService.AuthorizeAsync(User, id, (AuthorizationRequirement) HttpContext.Items["UserRequirements"])
					.Result)
			{
		...
		/// <summary>
		///     Finds a list of GameData with the highest
		///     <param name="dataType" />
		///     for each
		///     <param name="key" />
		...
		public IActionResult GetHighest(int? actorId, int? gameId, string[] key, EvaluationDataType dataType)
		{
				...
				|| _authorizationService.AuthorizeAsync(User, gameId, (AuthorizationRequirement) HttpContext.Items["GameRequirements"]).Result)
			{
				...
				var dataContract = dataList.ToCollectionContract();
				return new ObjectResult(dataContract);
			}
			return Forbid();
		}
		...
		public IActionResult Add([FromBody] EvaluationDataRequest newData)
		{
				...
				|| _authorizationService.AuthorizeAsync(User, newData.GameId, (AuthorizationRequirement) HttpContext.Items["GameRequirements"]).Result)
			{
				...
				var dataContract = EvaluationDataExtensions.ToContract(data);
				return new ObjectResult(dataContract);
			}
			return Forbid();
		}
