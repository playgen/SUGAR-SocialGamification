...
namespace PlayGen.SUGAR.WebAPI.Controllers
{
	...
	public class GroupMemberController : Controller
	{
		...
		{
			if (_authorizationService.AuthorizeAsync(User, groupId, (AuthorizationRequirement) HttpContext.Items["Requirements"])
				.Result)
			{
				var users = _groupMemberCoreController.GetMemberRequests(groupId);
				var actorContract = ActorExtensions.ToCollectionContract(users);
		...
		public IActionResult GetSentRequests([FromRoute] int userId)
		{
				...
				.Result)
			{
				...
				var actorContract = ActorExtensions.ToCollectionContract(requests);
				return new ObjectResult(actorContract);
			}
			return Forbid();
		}
		...
		public IActionResult GetMembers([FromRoute] int groupId)
		{
			...
			var actorContract = ActorExtensions.ToCollectionContract(members);
			return new ObjectResult(actorContract);
		}
		...
		public IActionResult GetUserGroups([FromRoute] int userId)
		{
			...
			var actorContract = ActorExtensions.ToCollectionContract(groups);
			return new ObjectResult(actorContract);
		}
