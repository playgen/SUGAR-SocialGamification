...
using EvaluationDataController = PlayGen.SUGAR.Data.EntityFramework.Controllers.EvaluationDataController;
using GroupController = PlayGen.SUGAR.Data.EntityFramework.Controllers.GroupController;
using LeaderboardController = PlayGen.SUGAR.Core.Controllers.LeaderboardController;
using UserController = PlayGen.SUGAR.Data.EntityFramework.Controllers.UserController;
...
namespace PlayGen.SUGAR.Core.UnitTests
{
	...
	{
		private readonly LeaderboardController _leaderboardCoreController =
			ControllerLocator.LeaderboardController;

		private readonly Data.EntityFramework.Controllers.LeaderboardController _leaderboardDbController =
			DbControllerLocator.LeaderboardController;

		private readonly EvaluationDataController _evaluationDataDbController =
			DbControllerLocator.EvaluationDataController;
		...
		private readonly GroupRelationshipController _groupRelationshipDbController =
			DbControllerLocator.GroupRelationshipController;

		public void GetLeaderboardSumLongAccuracyTest()
		{
			var leaderboard = CreateLeaderboard("GetLeaderboardSumLongAccuracyTest",
				EvaluationDataType.Long,
				LeaderboardType.Cumulative,
			...
			var filter = CreateLeaderboardStandingsRequest(leaderboard.Token,
				leaderboard.GameId,
				LeaderboardFilterType.Top,
				TestDataFixture.UserCount);

			var standings = _leaderboardCoreController.GetStandingsList(leaderboard, filter)
				.ToArray();
			...
			for (var i = 0; i < 5; i++)
			{
				...
				var randomId = random.Next(1, TestDataFixture.UserCount + 1);
				Assert.Equal(randomId,
					standings[filter.PageLimit - randomId]
						.ActorId);
			}
		}
		...
		private void CreateGroupMembership(string name, IEnumerable<User> users)
		{
			...
			group = _groupDbController.Get(group.Name)
				.ElementAt(0);
		...
		private IEnumerable<User> CreateUsers(int count, string uniqueNamePrefix)
		{
			...
			for (var i = 0; i < count; i++)
			{
				...
				user = _userDbController.Search(user.Name, true)
					.ElementAt(0);
				users.Add(user);
			}
		...
		private Leaderboard CreateLeaderboard(string name, EvaluationDataType dataType, LeaderboardType boardType,
			string customKey = "")
		{
			var random = new Random();
		...
		private LeaderboardStandingsRequest CreateLeaderboardStandingsRequest(string token, int gameId,
			LeaderboardFilterType filterType, int limit, int actorId = 0)
		{
		...
		private void CreateEvaluationData(IEnumerable<User> users, string key, EvaluationDataType type, int gameId = 0,
			bool singular = false)
		{
		...
		private void CreateEvaluationData(int userId, string key, EvaluationDataType type, int gameId = 0,
			bool singular = false)
		{
		...
		{
			for (var i = 1; i <= TestDataFixture.UserCount; i++)
			{
				var data = new List<EvaluationData>();
				for (var j = i; j > 0; j--)
				{
		...
		{
			var leaderboard = CreateLeaderboard("CreateFriendsLeaderboardStandingRequestWithInvalidActorId",
				EvaluationDataType.Long,
				LeaderboardType.Cumulative);
			...
			Assert.Throws<ArgumentException>(() => _leaderboardCoreController.GetStandingsList(leaderboard, filter));
		}
		...
		{
			var random = new Random();
			var leaderboard = CreateLeaderboard("CreateFriendsLeaderboardStandingRequestWithInvalidActorType",
				EvaluationDataType.Long,
				LeaderboardType.Cumulative);
			...
			var filter = CreateLeaderboardStandingsRequest(leaderboard.Token,
				...
				LeaderboardFilterType.Friends,
				0,
				random.Next(TestDataFixture.UserCount + 1, TestDataFixture.UserCount + TestDataFixture.GroupCount + 1));
		...
		{
			var leaderboard = CreateLeaderboard("CreateFriendsLeaderboardStandingRequestWithNoActorId",
				EvaluationDataType.Long,
				LeaderboardType.Cumulative);
		...
		public void CreateLeaderboardStandingRequestWithInvalidLeaderboardId()
		{
			...
			Assert.Throws<MissingRecordException>(() => _leaderboardCoreController.GetStandingsList(null, filter));
		}

		[Fact]
		public void CreateLeaderboardStandingRequestWithInvalidLimit()
		{
			var leaderboard = CreateLeaderboard("CreateLeaderboardStandingRequestWithInvalidLimit",
				EvaluationDataType.Long,
				LeaderboardType.Cumulative);
		...
		{
			var leaderboard = CreateLeaderboard("CreateMembersLeaderboardStandingRequestWithInvalidActorId",
				EvaluationDataType.Long,
				LeaderboardType.Cumulative);

			var filter = CreateLeaderboardStandingsRequest(leaderboard.Token,
				leaderboard.GameId,
				LeaderboardFilterType.GroupMembers,
				TestDataFixture.UserCount,
				-1);
		...
		{
			var random = new Random();
			var leaderboard = CreateLeaderboard("CreateMembersLeaderboardStandingRequestWithInvalidActorType",
				EvaluationDataType.Long,
				LeaderboardType.Cumulative);
			...
			var filter = CreateLeaderboardStandingsRequest(leaderboard.Token,
				...
				LeaderboardFilterType.GroupMembers,
				0,
				random.Next(1, TestDataFixture.UserCount + 1));
		...
		{
			var leaderboard = CreateLeaderboard("CreateMembersLeaderboardStandingRequestWithNoActorId",
				EvaluationDataType.Long,
				LeaderboardType.Cumulative);
		...
		{
			var leaderboard = CreateLeaderboard("CreateNearLeaderboardStandingRequestWithInvalidActorId",
				EvaluationDataType.Long,
				LeaderboardType.Cumulative);
		...
		{
			var random = new Random();
			var leaderboard = CreateLeaderboard("CreateNearLeaderboardStandingRequestWithInvalidActorType",
				EvaluationDataType.Long,
				LeaderboardType.Cumulative);
		...
		{
			var leaderboard = CreateLeaderboard("CreateNearLeaderboardStandingRequestWithNoActorId",
				EvaluationDataType.Long,
				LeaderboardType.Cumulative);
		...
		{
			var leaderboard = CreateLeaderboard("GetLeaderboardCountBoolAccuracyTest",
				EvaluationDataType.Boolean,
				LeaderboardType.Count,
				"GetLeaderboardCountBoolAccuracyTest");
		...
		{
			var leaderboard = CreateLeaderboard("GetLeaderboardCountBoolSpeedTest",
				EvaluationDataType.Boolean,
				LeaderboardType.Count);
		...
		{
			var leaderboard = CreateLeaderboard("GetLeaderboardCountStringAccuracyTest",
				EvaluationDataType.String,
				LeaderboardType.Count,
				"GetLeaderboardCountStringAccuracyTest");
		...
		{
			var leaderboard = CreateLeaderboard("GetLeaderboardCountStringSpeedTest",
				EvaluationDataType.String,
				LeaderboardType.Count);
		...
		{
			var leaderboard = CreateLeaderboard("GetLeaderboardEarliestBoolAccuracyTest",
				EvaluationDataType.Boolean,
				LeaderboardType.Earliest,
				"GetLeaderboardEarliestBoolAccuracyTest");
		...
		{
			var leaderboard = CreateLeaderboard("GetLeaderboardEarliestBoolSpeedTest",
				EvaluationDataType.Boolean,
				LeaderboardType.Earliest);
		...
		{
			var leaderboard = CreateLeaderboard("GetLeaderboardEarliestStringAccuracyTest",
				EvaluationDataType.String,
				LeaderboardType.Earliest,
				"GetLeaderboardEarliestStringAccuracyTest");
		...
		{
			var leaderboard = CreateLeaderboard("GetLeaderboardEarliestStringSpeedTest",
				EvaluationDataType.String,
				LeaderboardType.Earliest);
		...
		{
			var random = new Random();
			var leaderboard = CreateLeaderboard("GetLeaderboardFriendFilterAccuracyTest",
				EvaluationDataType.Long,
			...
			var filter = CreateLeaderboardStandingsRequest(leaderboard.Token,
				...
				LeaderboardFilterType.Friends,
				TestDataFixture.UserCount,
				random.Next(TestDataFixture.FriendCount + 1, TestDataFixture.UserCount - TestDataFixture.FriendCount + 1));
			...
			for (var i = 0; i < TestDataFixture.FriendCount * 2 + 1; i++)
				Assert.Equal(filter.ActorId.Value - TestDataFixture.FriendCount + i,
					standings[i]
						.ActorId);
		}
		...
		{
			var random = new Random();
			var leaderboard = CreateLeaderboard("GetLeaderboardFriendFilterSpeedTest",
				EvaluationDataType.Long,
				LeaderboardType.Cumulative);
		...
		public void GetLeaderboardGroupMemberFilterAccuracyTest()
		{
			...
			var standings = _leaderboardEvaulationController.GetStandingsList(leaderboard, filter).ToArray();
		...
		{
			var random = new Random();
			var leaderboard = CreateLeaderboard("GetLeaderboardGroupMemberFilterSpeedTest",
				EvaluationDataType.Long,
				LeaderboardType.Cumulative);
		...
		{
			var leaderboard = CreateLeaderboard("GetLeaderboardHighFloatAccuracyTest",
				EvaluationDataType.Float,
				LeaderboardType.Highest,
				"GetLeaderboardHighFloatAccuracyTest");
		...
		{
			var leaderboard = CreateLeaderboard("GetLeaderboardHighFloatSpeedTest",
				EvaluationDataType.Float,
				LeaderboardType.Highest);
		...
		{
			var leaderboard = CreateLeaderboard("GetLeaderboardHighLongAccuracyTest",
				EvaluationDataType.Long,
				LeaderboardType.Highest,
				"GetLeaderboardHighLongAccuracyTest");
		...
		{
			var leaderboard = CreateLeaderboard("GetLeaderboardHighLongSpeedTest",
				EvaluationDataType.Long,
				LeaderboardType.Highest);
		...
		{
			var leaderboard = CreateLeaderboard("GetLeaderboardLatestBoolSpeedTest",
				EvaluationDataType.Boolean,
				LeaderboardType.Latest);
		...
		{
			var leaderboard = CreateLeaderboard("GetLeaderboardLatestStringSpeedTest",
				EvaluationDataType.String,
				LeaderboardType.Latest);
		...
		{
			var leaderboard = CreateLeaderboard("GetLeaderboardLowFloatAccuracyTest",
				EvaluationDataType.Float,
				LeaderboardType.Lowest,
		...
		{
			var leaderboard = CreateLeaderboard("GetLeaderboardLowFloatSpeedTest",
				EvaluationDataType.Float,
				LeaderboardType.Lowest);
		...
		{
			var leaderboard = CreateLeaderboard("GetLeaderboardLowLongAccuracyTest",
				EvaluationDataType.Long,
				LeaderboardType.Lowest,
				"GetLeaderboardLowLongAccuracyTest");
		...
		{
			var leaderboard = CreateLeaderboard("GetLeaderboardLowLongSpeedTest",
				EvaluationDataType.Long,
				LeaderboardType.Lowest);
		...
		//	var standings = _leaderboardCoreController.GetStandingsList(leaderboard, filter).ToArray();
		...
		{
			var random = new Random();
			var leaderboard = CreateLeaderboard("GetLeaderboardNearFilterAccuracyTest",
				EvaluationDataType.Long,
				LeaderboardType.Lowest,
				"GetLeaderboardNearFilterAccuracyTest");
			...
			{
				var offset = (filter.ActorId.Value - 1) / (TestDataFixture.UserCount / filter.PageLimit) * filter.PageLimit;
				Assert.Equal(offset + i + 1,
					standings[i]
						.ActorId);
			}
		}
		...
		{
			var random = new Random();
			var leaderboard = CreateLeaderboard("GetLeaderboardNearFilterSpeedTest",
				EvaluationDataType.Long,
				LeaderboardType.Cumulative);
		...
		{
			var leaderboard = CreateLeaderboard("GetLeaderboardSumFloatAccuracyTest",
				EvaluationDataType.Float,
				LeaderboardType.Cumulative,
				"GetLeaderboardSumFloatAccuracyTest");
		...
		{
			var leaderboard = CreateLeaderboard("GetLeaderboardSumFloatSpeedTest",
				EvaluationDataType.Float,
				LeaderboardType.Cumulative);
		...
		{
			var leaderboard = CreateLeaderboard("GetLeaderboardSumLongSpeedTest",
				EvaluationDataType.Long,
				LeaderboardType.Cumulative);
