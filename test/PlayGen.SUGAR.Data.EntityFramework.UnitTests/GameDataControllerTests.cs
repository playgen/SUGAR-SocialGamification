...
namespace PlayGen.SUGAR.Data.EntityFramework.UnitTests
{
	...
	public class EvaluationDataControllerTests
	{
		...
		private EvaluationData CreateEvaluationData(string key, int? gameId = null, int? userId = null,
			bool createNewGame = false, bool createNewUser = false)
		{
			...
			var userData = new EvaluationData
			{
				Key = key,
				GameId = gameId,
				ActorId = userId,
		...
		public void CreateAndGetGameGlobalEvaluationData()
		{
			...
			var userDatas = _evaluationDataController.Get(newEvaluationData.GameId,
				newEvaluationData.ActorId,
				new List<string>
				{
					newEvaluationData.Key
				});

			var matches = userDatas.Count(g => g.Key == newEvaluationData.Key && g.GameId == newEvaluationData.GameId &&
												g.ActorId == newEvaluationData.ActorId);
		...
		public void CreateAndGetUserGameEvaluationData()
		{
			...
			var matches = userDatas.Count(g => g.Key == userDataName && g.GameId == newEvaluationData.GameId &&
												g.ActorId == newEvaluationData.ActorId);
		...
		public void CreateAndGetUserGlobalEvaluationData()
		{
			...
			var matches = userDatas.Count(g => g.Key == userDataName && g.GameId == null &&
												g.ActorId == newEvaluationData.ActorId);
		...
		public void GetUserEvaluationDatasWithNonExistingGame()
		{
			...
			var userDatas = _evaluationDataController.Get(-1,
				newEvaluationData.ActorId,
				new List<string>
				{
					userDataName
