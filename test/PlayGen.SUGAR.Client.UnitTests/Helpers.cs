	...
	public static class Helpers
	{
		public static ActorResponse GetOrCreateUser(UserClient userClient, string suffix = null)
		{
			var name = GetCallingClassName() + (suffix ?? $"_{suffix}") +
						"_User"; // "_User" suffix as temporary fix while lack of TPH in EF.Core means types inheriting from Actor cannot have the same "Name" value.
		...
		{
			var name = GetCallingClassName() + (suffix ?? $"_{suffix}") +
						"_Group"; // "_Group" suffix as temporary fix while lack of TPH in EF.Core means types inheriting from Actor cannot have the same "Name" value.
		...
		{
			var method = new StackTrace().GetFrame(2)
				.GetMethod();
