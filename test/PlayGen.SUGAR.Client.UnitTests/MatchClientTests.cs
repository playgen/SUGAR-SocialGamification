...
namespace PlayGen.SUGAR.Client.UnitTests
{
	...
	public class MatchClientTests : ClientTestsBase
	{
		...
		private void LoginUserForGame(string key = "MatchClientTests")
		{
			...
			try
			{
				_account = SUGARClient.Session.Login(_game.Id,
					new AccountRequest
					{
						Name = key,
			...
			{
				_account = SUGARClient.Session.CreateAndLogin(_game.Id,
					new AccountRequest
					{
						Name = key,
						Password = key + "Password",
		...
		private List<MatchResponse> StartAndEndMatches(int count)
		{
			...
			matches = matches.Select(m => SUGARClient.Match.End(m.Id))
				.ToList();
			return matches;
		}
		...
		public void CanAddAndGetData()
		{
			...
			// Assert
			datas.ForEach(a => Assert.True(got.Items.Any(g => g.GameId == a.GameId
														&& g.MatchId == a.MatchId
		...
		public void CanGetByCreator()
		{
			...
			// Assert
			got.Items.ToList().ForEach(m => Assert.AreEqual(_account.User.Id, m.Creator.Id));
			shouldGet.ForEach(m => Assert.IsTrue(got.Items.Any(g => g.Id == m.Id)));
			shouldntGet.ForEach(m => Assert.IsFalse(got.Items.Any(g => g.Id == m.Id)));
		}
		...
		public void CanGetByGameAndCreator()
		{
			...
			// Assert
			got.Items.ToList().ForEach(m => Assert.AreEqual(_game.Id, m.Game.Id));
			got.Items.ToList().ForEach(m => Assert.AreEqual(_account.User.Id, m.Creator.Id));
			shouldGet.ForEach(m => Assert.IsTrue(got.Items.Any(g => g.Id == m.Id)));
			shouldntGet.ForEach(m => Assert.IsFalse(got.Items.Any(g => g.Id == m.Id)));
		}
